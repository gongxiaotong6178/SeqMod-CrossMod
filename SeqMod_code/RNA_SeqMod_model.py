...
# -------------------------
# Linear with Shortcut (output head)
# -------------------------
...
# -------------------------
# Transformer + RoPE (single-feature)
# -------------------------
class TransformerWithROPE(nn.Module):
    def __init__(self, esm_dim=1280, proj_dim=256, nhead=8, dim_feedforward=1024):
        super().__init__()
        # trainable projection to proj_dim
        self.proj_esm = nn.Linear(esm_dim, proj_dim)
        self.total_dim = proj_dim

        self.nhead = nhead
        if self.total_dim % self.nhead != 0:
            raise ValueError(f"total_dim ({self.total_dim}) must be divisible by nhead ({self.nhead}).")
        self.head_dim = self.total_dim // nhead
    ...
    def forward(self, esm_feature):
        # esm_feature: (B,L,1280)
        x = self.proj_esm(esm_feature)  # project to 256 dim
...
if __name__ == "__main__":
    ...
    model = TransformerWithROPE(esm_dim=1280, proj_dim=256, nhead=8)
    out = model(esm)
    print("out.shape:", out.shape)  # expected (B,L,1)
