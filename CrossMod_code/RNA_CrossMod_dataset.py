...
class BindingDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        rna_label = np.array(
            [[int(x)] for x in rna_annotation],
            dtype=np.int64
        )

        rna_label = torch.as_tensor(
            rna_label,
            dtype=torch.float32
        )

        return esm2_feat, saprot_feat, rna_label
...
def check_dataloader(dataloader):
    for i, data in enumerate(dataloader):
        ...
        esm_feat, saprot_feat, rna_label = data
...
if __name__ == "__main__":
    ...
    test117_dataloader = create_dataloader(
        fasta_path="TEST117_FASTA",
        saprot_dir=saprot_feature_dir,
        esm2_dir=esm2_feature_dir
    )
